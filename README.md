...
- **Vues** (menu en haut à gauche) : *Tout l'arbre*, *Descendance de…*, *Ascendance de…* (les ancêtres d'une personne) ou *Ascendance et descendance*. Un 2ème menu limite le nombre de générations affichées à partir de cette personne (1 à 8, ou *Toutes*) : pratique pour un arbre qui va grandir, en évitant d'afficher des dizaines de générations d'un coup. Les boutons « Sa descendance » / « Ses ancêtres » d'une fiche font pareil (sans limite de générations). L'adresse contient `#vue=…`, on peut la partager. Pour un gros arbre, c'est la vue *Ascendance de…* avec un nombre de générations limité qui donne un bel arbre « pedigree » lisible.
